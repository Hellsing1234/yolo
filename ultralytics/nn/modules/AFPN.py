...
from ultralytics.nn.modules.block import C2f,C2f_EMBC
...
class ASFF_2(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[256, 512]): 
        #inter_dim特征图的维度  level: 表示当前模块处理的特征层级，0 或 1。channel: 表示不同层级的特征通道数。
        super(ASFF_2, self).__init__()

        self.inter_dim = inter_dim
        compress_c = 8
        #weight_level_1 和 weight_level_2: 两个卷积层，用于生成不同层级特征的权重。
        self.weight_level_1 = Conv(self.inter_dim, compress_c, 1, 1)
        self.weight_level_2 = Conv(self.inter_dim, compress_c, 1, 1)
       # weight_levels: 一个1x1的卷积层，用于将两个层级的权重特征合并，并生成最终的权重。
        ...
        self.level = level
        self.c2f = C2f_EMBC(inter_dim, inter_dim)

    def forward(self, x):
        #根据 level 的值，对输入特征 input2 进行上采样或 input1 进行下采样，以匹配特征尺寸。
        input1, input2 = x
        if self.level == 0:
            input2 = self.upsample(input2)
        elif self.level == 1:
            input1 = self.downsample(input1)
        #使用 weight_level_1 和 weight_level_2 分别处理两个输入特征，生成权重特征。
        level_1_weight_v = self.weight_level_1(input1)
        level_2_weight_v = self.weight_level_2(input2)
         #将两个层级的权重特征在通道维度上进行拼接。
        levels_weight_v = torch.cat((level_1_weight_v, level_2_weight_v), 1)
        #通过 weight_levels 卷积层和 softmax 激活函数生成最终的权重。
        levels_weight = self.weight_levels(levels_weight_v)
        levels_weight = F.softmax(levels_weight, dim=1)
        #使用生成的权重对不同层级的特征进行加权融合。
        fused_out_reduced = input1 * levels_weight[:, 0:1, :, :] + \
                            input2 * levels_weight[:, 1:2, :, :]
        #将融合后的特征通过 conv 卷积层进行处理。
